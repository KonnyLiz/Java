...
import clasesBdd.ventasBdd;

public class dialogMostrarEstadoComprasVentas extends JFrame {
	 ...
	 */
	public dialogMostrarEstadoComprasVentas(String fecha1, String fecha2) {
		super();
		initialize();
	 ...
	 * @param Query
	 */
	public void llenarReporte(HashMap parameterMap, String Query) {
		...
		try {
			...
			reporte = (JasperReport) JRLoader
					.loadObject("bin/reportes/reporteEstadoGeneralComprasVentas.jasper");

			JasperPrint jasperPrint = JasperFillManager.fillReport(reporte,
					parameterMap, conn.getConexion());//
			JasperViewer jviewer = new JasperViewer(jasperPrint, false);
			jviewer.setTitle("Reporte Estado General de compras-ventas");
	...
	public void llamarCompraporGastoReporte(String fecha1, String fecha2) {
		ventasBdd ventasMd = new ventasBdd();
		conexionBdd conn = new conexionBdd();

		Float GastosTotales = ventasMd.GastosTotales(conn.getConexion(),
				fecha1, fecha2);
		Float ivaCobrado = ventasMd.ivaCobrado(conn.getConexion(), fecha1,
				fecha2);
		Float ivaPagado = ventasMd
				.ivaPagado(conn.getConexion(), fecha1, fecha2);
		Float pagosAPlazo = ventasMd.pagosAPlazo(conn.getConexion(), fecha1,
				fecha2);
		Float VentasAPlazo = ventasMd.VentasAPlazo(conn.getConexion(), fecha1,
				fecha2);
		Float ventasFacturas = ventasMd.ventasFacturas(conn.getConexion(),
				fecha1, fecha2);
		Float ventasSaldoPorCobrarODT = ventasMd.ventasSaldoPorCobrarODT(conn
				.getConexion(), fecha1, fecha2);
		Float ventasTotalODT = ventasMd.ventasTotalODT(conn.getConexion(),
				fecha1, fecha2);
		utilitarios util = new utilitarios();
		Float resta2 = Float.parseFloat(util.cortarCadenaFloat(""
				+ (ivaCobrado - ivaPagado)));
		Float resta1 = Float.parseFloat(util.cortarCadenaFloat(""
				+ (ventasTotalODT - GastosTotales)));


		HashMap parameterMap = new HashMap();
		parameterMap.put("fecha1", fecha1);
		parameterMap.put("fecha2", fecha2);
		parameterMap.put("gastosTotales", GastosTotales);
		parameterMap.put("ivaCobrado", ivaCobrado);
		parameterMap.put("ivaPagado", ivaPagado);
		parameterMap.put("pagosAPlazo", pagosAPlazo);
		parameterMap.put("VentasAPlazo", VentasAPlazo);
		parameterMap.put("ventasFacturas", ventasFacturas);
		parameterMap.put("ventasSaldoPorCobrarODT", ventasSaldoPorCobrarODT);
		parameterMap.put("ventasTotalODT", ventasTotalODT);
		parameterMap.put("resta1", resta1);
		parameterMap.put("resta2", resta2);

		String Query = "SELECT  " + "bancos.DESCRIPCION_BANCO" + " FROM bancos";

		llenarReporte(parameterMap, Query);
	}
