...
public class dialogMostrarRecibosPorCobrar extends JFrame {
	 ...
	 */
	public dialogMostrarRecibosPorCobrar(String fecha1, String fecha2) {
		super();
		initialize();
		...
		try {
			...
			if (resultSet.getString(resultSet.getRow()) != null) {
				...
				reporte = (JasperReport) JRLoader
						.loadObject("bin/reportes/reporteFacturasPorCobrar.jasper");
				...
				jviewer.setTitle("Reporte Recibos por Cobrar");
				jviewer.setVisible(true);
				resultSet.close();
			} else {
				JOptionPane.showMessageDialog(null,
						"Esta consulta no tiene registros! ", "Aviso!",
	...
	public void llamarCompraporGastoReporte(String fecha1, String fecha2) {
		String Query = "SELECT " + "     R.`NUMERO_RECIBO`,"
				+ "     R.`ID_OT`," + "     R.`CI_RUC`," + "     C.`NOMBRE`,"
				+ "     x.`DESCRIPCION_TR`," + "     R.`FECHA`,"
				+ "     R.`TOTAL`,   " + "     SUM(CANTIDAD)," + "     PLAZO"
				+ "     FROM " + "     `recibo_venta` R, "
				+ "     `tipo_pago_recibo` t, " + "     `tipo_recibo` x, "
				+ "     `cliente` C " + " WHERE "
				+ "     R.ID_RECIBO = t.ID_RECIBO " + " AND x.ID_TR = R.ID_TR "
				+ "     and c.CI_RUC = R.CI_RUC " + " AND R.`FECHA` BETWEEN '"
				+ fecha1 + "' AND '" + fecha2 + "'";
