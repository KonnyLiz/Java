...
public class dialogMostrarProdCategoria extends JFrame {
	 ...
	 */
	public dialogMostrarProdCategoria(String fecha1, String fecha2,String categoria) {
		super();
		initialize();
		this.setDefaultCloseOperation(JFrame.EXIT_ON_CLOSE);
		fecha1Init = fecha1;
		fecha2Init = fecha2;
		categoriaInit = categoria;
	...
	class MiWorker extends SwingWorker<String, Void> {
		...
		protected String doInBackground() throws Exception {
			...
			while (i <= 100) {
				if (starter) {
					llamarCompraporGastoReporte(fecha1Init, fecha2Init,categoriaInit);
					starter = false;
				}
	...
	public void llenarReporteEgresos(HashMap parameterMap, String Query) {
		...
		try {
			...
			reporte = (JasperReport) JRLoader
					.loadObject("bin/reportesnuevos/reporteInventarioCategoria.jasper");
			...
			jviewer.setTitle("Reporte productos por categoría");
	 ...
	 * @param fecha2
	 */
	public void llamarCompraporGastoReporte(String fecha1, String fecha2,String categoria) {
		String Query = " select" +
				" p.ID_PRO, p.ID_PROV," +
				" (SELECT NOMENCLATURA FROM unidad_medida where ID_UNIDAD_MEDIDA= p.ID_UNIDAD_MEDIDA) ," +
				"  p.DESCRIPCION, p.PVP,(SELECT sum(TOTAL_BRUTO) FROM inventario where ID_PRO=p.ID_PRO) " +
				" from producto p " +
				" where p.ID_TP = (SELECT ID_TP FROM tipo_producto WHERE DESCRIPCION_TP='"+categoria+"')";
