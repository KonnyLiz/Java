...
public class dialogMostrarVentasPorTipoCliente extends JFrame {
	 ...
	 */
	public dialogMostrarVentasPorTipoCliente(String fecha1, String fecha2) {
		super();
		initialize();
	...
	public void llenarReporteEgresos(HashMap parameterMap, String Query) {
		...
		try {
			 ...
			 * parameterMap, conn.getConexion());// JasperViewer jviewer = new
			 * JasperViewer(jasperPrint,false); jviewer.setTitle("Reporte de la
			 * Orden De Trabajo"); jviewer.setVisible(true);
			...
			reporte = (JasperReport) JRLoader
					.loadObject("bin/reportes/reportClienteODTFechas.jasper");

			JasperPrint jasperPrint = JasperFillManager.fillReport(reporte,
					parameterMap, conn.getConexion());//
			JasperViewer jviewer = new JasperViewer(jasperPrint, false);
			jviewer
					.setTitle("Reporte Ventas Ordenes de trabajo por tipo de Cliente");
	...
	public void llamarCompraporGastoReporte(String fecha1, String fecha2) {
		String Query = "SELECT" + " distinct o.CI_RUC," + " count(o.CI_RUC),"
				+ "t.DESCRIPCION," + "o.FECHA" + ",c.NOMBRE,"
				+ "TRUNCATE(sum(precio),2)" + " FROM order_trabajo "
				+ "	   o,tipo_cliente " + "      t,cliente c"
				+ " where o.CI_RUC=c.CI_RUC " + "       and t.id_tc=c.id_tc "
				+ "and " + "    o.FECHA BETWEEN '" + fecha1 + "' and '"
				+ fecha2 + "'" + "group by 1 order by descripcion";
