...
public class tipoProductoVentana extends JInternalFrame {
	...
	private tipoProducto tipoProductoDp = new tipoProducto(0, ""); // @jve:decl-index=0:
	...
	private void initialize() {
		...
		this.setTitle("Administración Tipos de Producto");
	}
	...
	private JPanel getJContentPane() {
		if (jContentPane == null) {
			...
			Administracióndeclientes
					.setText("Administración de Tipos de Producto");
			Administracióndeclientes
	...
	private JPanel getJPanelEtiquetas() {
		if (jPanelEtiquetas == null) {
			...
			jLabelDescripcion.setText("ID de Tipo de Producto :");
	...
	private JTextField getJTextFieldDescripcion() {
		if (jTextFieldDescripcion == null) {
			...
			jTextFieldDescripcion
					.setToolTipText("Modifica la Información del Tipo de Producto");
			jTextFieldDescripcion
					.addFocusListener(new java.awt.event.FocusAdapter() {
						...
						public void focusLost(java.awt.event.FocusEvent e) {
							if (jTextFieldDescripcion.getText().trim()
								...
								tipoProductoDp
										.setDescripcionTipoProducto(jTextFieldDescripcion
												.getText().trim());
								buscarItem();
							}// end else
						}
		...
		if (choiceExistentes == null) {
			...
			choiceExistentes.addItemListener(new java.awt.event.ItemListener() {
				public void itemStateChanged(java.awt.event.ItemEvent e) {
					...
					tipoProductoDp.setDescripcionTipoProducto(choiceExistentes
							.getSelectedItem());
					opcionModulo = 2;
					buscarItem();
	...
	private void buscarItem() {
		...
		tipoProductoDp = tipoProductoMd.buscarInfoDeUnRegistro(con
				.getConexion(), tipoProductoDp);

		if (tipoProductoDp.getIdTipoProducto() == 0) {
			/**
			...
			jTextFieldId.setEnabled(false);
			choiceExistentes.setEnabled(false);
			jTextFieldId.setText("" + tipoProductoDp.getIdTipoProducto());
			jTextFieldDescripcion.setText(tipoProductoDp
					.getDescripcionTipoProducto());
			jButtonIngresar.setEnabled(false);
	...
	private void limpiar() {
		...
		opcionModulo = 0;
		tipoProductoDp = new tipoProducto(0, "");
	...
	private void insertar() {
		if (opcionModulo == 1
			...
			tipoProductoMd.insertar(con.getConexion(), tipoProductoDp);
			cargarChoice();
		} else {
			JOptionPane.showMessageDialog(null,
	...
	private void modificar() {
		if (opcionModulo == 2
				&& jTextFieldDescripcion.getText().compareTo("") != 0) {
			tipoProductoMd.modificar(con.getConexion(), tipoProductoDp);
			cargarChoice();
		} else {
			JOptionPane.showMessageDialog(null,
					"El campo Descripción Está Vacio! ", "Aviso!",
	...
	private void eliminar() {
		if (opcionModulo == 2
				&& jTextFieldDescripcion.getText().compareTo("") != 0) {
			tipoProductoMd.eliminar(con.getConexion(), tipoProductoDp);
			cargarChoice();
		} else {
			JOptionPane.showMessageDialog(null,
					"El campo Descripción Está Vacio! ", "Aviso!",
