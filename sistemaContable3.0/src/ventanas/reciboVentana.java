...
public class reciboVentana extends JInternalFrame {
//public class reciboVentana extends JFrame {
    ...
    reciboBdd reciboMd = new reciboBdd(); // @jve:decl-index=0:
    ...
    recibo reciboDP = new recibo(0, 0, 0, 0, 0, "", "", "", 0, 0, 0); // @jve:decl-index=0:
    int sigFactura = Integer.parseInt(reciboMd.seleccionarSigRecibo(conn
            .getConexion()));
    int sigNotaVenta = Integer.parseInt(reciboMd.seleccionarNotaVenta(conn
            .getConexion()));
    /**
     * parametros para impresión
     */
    proforma proformaDP = new proforma(0, "", "", "", 0, 0, 0, "");
    ...
    private JButton jButtonBuscarOdt = null;
    ...
    String User = "";// comentar
    String numeroOdt;
    boolean facturarOdt;
    String proformaNumero;
    boolean facturarProforma;
    private JButton jButtonBuscarCliente = null;
    boolean ventanaClienteAbierta = false;
    listaClientesRecibo lista = new listaClientesRecibo();  //  @jve:decl-index=0:visual-constraint="703,10"
    String siguienteIdReciboDetalle = reciboMd.devolverSiRegistroDetalle(conn
            .getConexion()); // @jve:decl-index=0:
    private JButton jButtonAnular = null;
    public boolean reciboExiste=false;
    public float TotalODTObtenido=0;
    public float sumatorioSubtotales=0;
    public int contatItems=0;
    /**
     * This is the default constructor
     */

    public reciboVentana(String numeroOdt1, boolean facturarOdt1,
            String proformaNumero1, boolean facturarProforma1, String User1,
        JDesktopPane JDesktopPanePrin1, Float totalrecibo) {

        super();
        JDesktopPanePrin = JDesktopPanePrin1;
        User = User1;// comentar
        numeroOdt = numeroOdt1;
        facturarOdt = facturarOdt1;
        proformaNumero = proformaNumero1;
        facturarProforma = facturarProforma1;
        initialize();
        this.setClosable(true);
        this.setIconifiable(true);
        verificarDatosExternos(numeroOdt1, facturarOdt1, proformaNumero1,
                facturarProforma1, totalrecibo);
    ...
    /*public reciboVentana() {

        super();
    //    JDesktopPanePrin = JDesktopPanePrin1;
        User = "qawsdaw";// comentar
        initialize();
        //this.setClosable(true);
        //this.setIconifiable(true);
*/
    ...
    private void initialize() {
        lista.setSize(new Dimension(654, 390));
        lista.setAlwaysOnTop(true);
        this.setSize(663, 639);
        this.setContentPane(getJContentPane());
        this.setTitle("Administración de Notas de Venta y Facturas");
    }
    ...
    private JPanel getJContentPane() {
        if (jContentPane == null) {
            ...
            jLabelDescripcion121.setBounds(new Rectangle(202, 240, 438, 21));
            ...
            Administraciónproveedor
                    .setText("Administración de Notas de Venta y Facturas");
            ...
            jContentPane.add(getJButtonBuscarOdt(), null);
            ...
            jContentPane.add(getJButtonAnular(), null);
        }
        return jContentPane;
    }
        ...
        if (choice == null) {
            ...
            choice.addItemListener(new java.awt.event.ItemListener() {
                public void itemStateChanged(java.awt.event.ItemEvent e) {
                    if(choice.getSelectedItem().compareToIgnoreCase("Escoja un opción")!=0){
     ...
     *
     */
    private void verificarDatosExternos(String numeroOdt1,
            boolean facturarOdt1, String proformaNumero1,
            boolean facturarProforma1, Float total) {

        proforma proformaDp = new proforma(0, "", "", "", 0, 0, 0, "");
        proformaBdd proformaMd = new proformaBdd();

        if (facturarOdt1) {// revisa si se ingreso odt por constructor

            choice.select(2);
            estado = choice.getSelectedItem();
            choice.setEnabled(true);
            jTextFieldOdt.setText(numeroOdt1);
            jButtonBuscarOdt.doClick();
            Object temp[][] = odtMd
                    .solicitarRuc(conn.getConexion(), numeroOdt1);
            try {
                jTextFieldRUC.setText("" + temp[0][0]);
            } catch (Exception a1) {
                System.out.print("Excepcion" + temp.length);
            }
            cliente clienteDP = new cliente(0, jTextFieldRUC.getText().trim(),
                    "", "", "", "", "", "");
            datoscliente(clienteDP);
            jButtonBuscarOdt.setEnabled(false);

        }
        if (facturarProforma1) {

            choice.select(2);
            estado = choice.getSelectedItem();
            choice.setEnabled(true);

            Object arreglo[][] = proformaMd.solicitarItems(conn.getConexion(),
                    proformaNumero1);
            if (arreglo.length == 0) {
                JOptionPane
                        .showMessageDialog(
                ...
                for (int i = 0; i < arreglo.length; i++) {
                    ...
                    cantidad = util.Redondear("" + cantidad, 4);
                    ...
                    Precio = util.Redondear("" + Precio, 4);
                    float subtotal = Precio * cantidad;
                    subtotal = util.Redondear("" + subtotal, 4);
            ...
            Object temp[][] = proformaMd.solicitarRuc(conn.getConexion(),
                    proformaNumero1);
            jTextFieldRUC.setText("" + temp[0][0]);
            cliente clienteDP = new cliente(0, jTextFieldRUC.getText().trim(),
                    "", "", "", "", "", "");
    ...
    private JScrollPane getJScrollPane() {
        if (jScrollPane == null) {
            ...
            jScrollPane.setBounds(new Rectangle(27, 321, 607, 127));
            ...
            jTable.addKeyListener(new java.awt.event.KeyAdapter() {
                public void keyReleased(java.awt.event.KeyEvent e) {
                    ...
                    float nuevaCantidad = (Float) modelo.getValueAt(
                            filaQueCambio, 0);
                    if (modelo.getRowCount() > 0
                            && cantidadSolicitada > devolverMaximoParaIngresar())
                        nuevaCantidad = devolverMaximoParaIngresar();
...
*/
    ...
    int pedirConfirmacionAnular() {
        int res = 1;
        res = JOptionPane.showConfirmDialog(this, "Desea Anular  el Recibo?. Esta acción no es reversible.",
    ...
    private JTextField getjTextFieldValor() {
        if (jTextFieldValor == null) {
            ...
            jTextFieldValor.setBounds(new Rectangle(561, 264, 74, 26));
    ...
    private JPanel getJPanel() {
        if (jPanel == null) {
            ...
            jPanel.setBounds(new Rectangle(5, 262, 151, 26));
                ...
                public void actionPerformed(java.awt.event.ActionEvent e) {
                    ...
                    if (producto.toString().compareTo("Escoja un Producto") != 0
                        ...
                        if (modelo.getRowCount() > 10) {
                            ...
                            if (!cantidadItemsIngresados()) {
                                JOptionPane
                                        .showMessageDialog(
                                                null,
                                                "La cantidad ingresada excede al disponible!! ",
                                                "Alerta!",
    ...
    private JPanel getJPanel22() {
        if (jPanel22 == null) {
            ...
            jPanel22.setBounds(new Rectangle(399, 451, 118, 110));
    ...
    private JPanel getJPanel221() {
        if (jPanel221 == null) {
            ...
            jPanel221.setBounds(new Rectangle(531, 451, 104, 109));
    ...
    private detalleRecibo validarItem() {
        ...
        try {
            ...
            precioUnitario = util.Redondear("" + precioUnitario, 4);
            float subT = cantidad * precioUnitario;
            subT = util.Redondear("" + subT, 4);
        ...
        if (jTextFieldRUC == null) {
            ...
            jTextFieldRUC.addFocusListener(new java.awt.event.FocusAdapter() {
                public void focusLost(java.awt.event.FocusEvent e) {
                    ...
                    System.out.println("ec1:" + clienteDP.getSexo());
        ...
        if (jTextFieldCliente == null) {
            ...
            jTextFieldCliente.addKeyListener(new java.awt.event.KeyAdapter() {
                public void keyReleased(java.awt.event.KeyEvent e) {
                    ...
                    ExisteCliente = false;
                    System.out.println("ec:" + clienteDP.getSexo());
                    if (clienteDP.getSexo().trim().compareTo("") != 0) {
    ...
    private JComboBox getChoiceTipoProducto() {
        if (choiceTipoProducto == null) {
            ...
            choiceTipoProducto.setBounds(new Rectangle(203, 266, 125, 21));
    ...
    private JComboBox getChoiceProductos() {
        if (choiceProductos == null) {
            ...
            choiceProductos.setBounds(new Rectangle(338, 266, 190, 21));
            choiceProductos
                    .addActionListener(new java.awt.event.ActionListener() {
                        public void actionPerformed(java.awt.event.ActionEvent e) {
    ...
    private void colocarPrecio() {
        ...
        jTextFieldValor.setText("" + util.Redondear("" + precio * cantidad, 4));
    ...
    private JTextField getJTextFieldUniodad() {
        if (jTextFieldUniodad == null) {
            ...
            jTextFieldUniodad.setBounds(new Rectangle(162, 266, 31, 21));
    ...
    private void colocarTotal() {
        ...
        //total=util.TruncarRedondear(total);//truncar a 2 cifras el total
        if (jCheckBoxAplicaIva.isSelected() && estado.compareTo("Factura") == 0) {
            total=util.TruncarRedondear(total);

            jTextFieldSubtotal.setText("" + total);

            jTextFieldIva0.setText("0");
            float iva1 = iva + 1;
            float totalConIva = util.TruncarRedondear(total * iva1);
            float ivaCampo = totalConIva - total;
            ivaCampo = util.TruncarRedondear(ivaCampo);
            jTextFieldIva12.setText("" + ivaCampo);
            totalConIva = util.Redondear(""+(total + ivaCampo),2);
            jTextFieldTotal.setText(""+ totalConIva);
    ...
    /**
     * This method initializes jButtonBuscarOdt
     *
     * @return javax.swing.JButton
     */
    private JButton getJButtonBuscarOdt() {
        if (jButtonBuscarOdt == null) {
            jButtonBuscarOdt = new JButton();
            jButtonBuscarOdt.setBounds(new Rectangle(288, 66, 79, 25));
            jButtonBuscarOdt.setText("Aceptar");
            jButtonBuscarOdt
                    .addActionListener(new java.awt.event.ActionListener() {
                        public void actionPerformed(java.awt.event.ActionEvent e) {
                            if (choice.getSelectedItem().compareTo(
                                    "Escoja un opción") == 0) {
                                choice.select("Factura");
                                estado = choice.getSelectedItem();
                            }
                            cargarItemsDatosOdt();
        ...
        return jButtonBuscarOdt;
    }
/**
*carga los datos del recibo en la grilla
*para mostrarlos al usuario
*  los datos provienen de la orden de trabajo
*/
    ...
    private void cargarItemsDatosOdt() {
        ...
        if (modelo.getRowCount() > 10)
            ...
            if (arreglo.length == 0) {
                ...
                TotalODTObtenido=odtMd.solicitarTotal(conn.getConexion(),jTextFieldOdt.getText().trim());
                contatItems=0;
                float iva=0;
                Object[][] arregloItems= new Object[10][4];
                jButtonBuscarOdt.setEnabled(false);
                for (int i = 0; i < arreglo.length; i++) {
                    float cantidad = Float.parseFloat("" + arreglo[i][0]);
                    cantidad = util.Redondear("" + cantidad, 2);
                    String descripcion = "" + arreglo[i][1];
                    iva = (Float.parseFloat("" + IvaImpresora[0][0]) / 100);
                    iva = iva + 1;
                    float Precio = Float.parseFloat(""+arreglo[i][2]) ;
                    Precio=util.TruncarRedondear(Precio);


                    /**
                     * Parche de diferencia decimal en el iva no es exacto
                     */

                    float subtotal = (Precio * cantidad)/iva;
                    subtotal = util.Redondear("" + subtotal, 3);
                    Precio=util.Redondear("" + subtotal/cantidad, 3);
                    //Precio=util.TruncarRedondear(Precio);
                    arregloItems[i][0]=cantidad;
                    arregloItems[i][1]=descripcion;
                    arregloItems[i][2]=Precio;
                    arregloItems[i][3]=subtotal;
                    sumatorioSubtotales+=subtotal;
                    contatItems+=1;
                    if(jButtonBuscarOdt.isEnabled())
                         poblarTabla(cantidad,descripcion, Precio, subtotal);

                }


                if(!jButtonBuscarOdt.isEnabled())
                    corregirSubotales(arregloItems,iva);
            }// else
        }// else
    ...
    private JPanel getJPanel4() {
        if (jPanel4 == null) {
            ...
            jPanel4.setBounds(new Rectangle(53, 574, 546, 26));
    ...
    private JButton getJButtonAceptar() {
        ...
        if (jButtonAceptar == null) {
            ...
            jButtonAceptar
                    .addActionListener(new java.awt.event.ActionListener() {
                        public void actionPerformed(java.awt.event.ActionEvent e) {
                            if (reciboExiste==false){
                            if (modelo.getRowCount() == 0) {
                                JOptionPane.showMessageDialog(null,
                                        "La Factura no tiene Items!", "Aviso",
                                        JOptionPane.ERROR_MESSAGE);
                            ...
                            JOptionPane.showMessageDialog(null,
                                    "Número de recibo incorrecto!", "Aviso",
                                    JOptionPane.ERROR_MESSAGE);


                        }       //if checar si el recibo esta ingresado
                        }
                    });
        }
            ...
            jButtonVerReporte
                    .addActionListener(new java.awt.event.ActionListener() {
                        public void actionPerformed(java.awt.event.ActionEvent e) {
                            ...
                            int AntRecibo=(Integer.parseInt(siguienteIdReciboDetalle));
//System.out.println(AntRecibo+":id recibo   it tipo:"+reciboDP.getIdTipoRecibo());
                            reporteJasper.recibirDatosExternos(AntRecibo, reciboDP.getIdTipoRecibo());
                        }
                    });
        }
    ...
    private JPanel getJPanel23() {
        if (jPanel23 == null) {
            ...
            jPanel23.setBounds(new Rectangle(371, 107, 154, 60));
    ...
    private JPanel getJPanel31() {
        if (jPanel31 == null) {
            ...
            jPanel31.setBounds(new Rectangle(534, 107, 103, 26));
                            ...
                            jTextFieldNumeroRecibo.setText(util
                                    .cortarCadenaFloat(""
                                            + jTextFieldNumeroRecibo.getText()
                                                    .trim()));
                            reciboDP.setNumeroRecibo(Integer
                                    .parseInt(jTextFieldNumeroRecibo.getText()
                                            .trim()));
                        }
    ...
    private void limpiar() {
        ...
        reciboVentana reciboVentanaW = new reciboVentana("", false, "", false,
                User, JDesktopPanePrin, new Float(0));
        // facturaVentana facturaVentanaW= new
        // facturaVentana(null,false,null,false);
        reciboVentanaW.getPreferredSize();
        reciboVentanaW.setLocation(this.getX(), this.getY());
        JDesktopPanePrin.add(reciboVentanaW);
        this.dispose();
    ...
    private int validar() {
        ...
        int estado1 = 0;
        reciboDP.setNumeroRecibo(Integer.parseInt(jTextFieldNumeroRecibo
                .getText().trim()));
        if (jTextFieldNumeroRecibo.getText().trim().compareTo("") != 0) {
            ...
            reciboDP.setCiRuc(clienteDp.getCiRuc());
            reciboDP.setCiudad("QUITO");
            reciboDP.setFecha(util.fechaActual());
            reciboDP.setIdOrdenTrabajo(Integer.parseInt(jTextFieldOdt.getText()
                    .trim()));
            reciboDP.setIdRecibo(Integer.parseInt(reciboMd
                    .seleccionarSigRecibo(conn.getConexion())));
            if (choice.getSelectedItem().compareTo("Factura") == 0) {
                reciboDP.setIdTipoRecibo(2);
            } else {
                reciboDP.setIdTipoRecibo(1);
            }
            user userDp = new user(columnTemp, columnTemp, estado, User, estado);
            userDp = userMd.buscarInfoDeUnRegistro(conn.getConexion(), userDp);
            reciboDP.setIdUser(userDp.getIdUser());
            reciboDP.setIva(Float.parseFloat(jTextFieldIva12.getText().trim()));
            reciboDP.setSubtotal(Float.parseFloat(jTextFieldSubtotal.getText()
                    .trim()));
            reciboDP.setTotal(Float
                    .parseFloat(jTextFieldTotal.getText().trim()));
            reciboDP.setNumeroRecibo(Integer.parseInt(jTextFieldNumeroRecibo
                    .getText().trim()));
            reciboMd.insertar(conn.getConexion(), reciboDP);
            proformaDP.setCiRuc(reciboDP.getCiRuc());
            proformaDP.setCiudad("QUITO");
            proformaDP.setEstado("");
            proformaDP.setFecha(reciboDP.getFecha());
            proformaDP.setIdProforma(1);
            proformaDP.setIva(reciboDP.getIva());
            proformaDP.setSubtotal(reciboDP.getSubtotal());
            proformaDP.setTotal(reciboDP.getTotal());

            modificarAfectados(facturarProforma, facturarOdt);

            for (int i = 0; i < modelo.getRowCount(); i++) {
                float cantidad = (Float) modelo.getValueAt(i, 0);
                reciboDetalle facturaDetalleDp = new reciboDetalle(0, 0, "", 0,
                        0, 0);
                facturaDetalleDp.setIdRecibo(Integer
                        .parseInt(siguienteIdReciboDetalle)); // error
                facturaDetalleDp.setIdDescripcion((String) modelo.getValueAt(i,
                        1));
                facturaDetalleDp.setPrecio((Float) modelo.getValueAt(i, 2));
                facturaDetalleDp.setIdMedidaX(0);
                facturaDetalleDp.setIdMedidaY(0);
                arregloProductosTempHash productoSolicitado = listaProductosDescripcionArreglo
                        .get(facturaDetalleDp.getIdDescripcion());
                boolean temp = listaProductosDescripcionArreglo.keySet()
                        .contains(facturaDetalleDp.getIdDescripcion());
                final String c = "" + temp;
                if (c.charAt(0) != 'f') {

                    facturaDetalleDp.setIdProducto(productoSolicitado
                            .getIdProd());
                    //System.out.print("\nsi tiene:" + c.charAt(0) + "::"
                            //+ facturaDetalleDp.getIdProducto());
                } else {
                    facturaDetalleDp.setIdProducto(0);
                    //System.out.print("\nno Existe:" + c.charAt(0) + "::"
                        //    + facturaDetalleDp.getIdProducto());
                ...
                proformaDetalleDP[i].setCantidad((Float) modelo
                        .getValueAt(i, 0));
                proformaDetalleDP[i].setIdDescripcion(facturaDetalleDp
                        .getIdDescripcion());
                proformaDetalleDP[i].setIdPro(facturaDetalleDp.getIdProducto());
                proformaDetalleDP[i].setTotal((Float) modelo.getValueAt(i, 3));

                System.out.print("\n\n---id pro:"
                        + proformaDetalleDP[i].getIdPro());
                if (proformaDetalleDP[i].getIdPro() != 0)

                    restarInventario(facturaDetalleDp, cantidad);
                for (int j = 0; j < cantidad; j++)
                    estado1 = reciboMd.insertarDetalle(conn.getConexion(),
                            facturaDetalleDp);

            }
            enviarPago(5, reciboDP.getNumeroRecibo(), "" + reciboDP.getTotal(),
                    "" + reciboDP.getFecha(), reciboDP.getIdTipoRecibo());
        } else {
            JOptionPane.showMessageDialog(null,
    ...
    private void sugerirNumero() {
        if (estado.compareTo("Factura") == 0)
            jTextFieldNumeroRecibo.setText("" + sigFactura);
        else
            jTextFieldNumeroRecibo.setText("" + sigNotaVenta);
    }

    /**
     * busca un cliente y setea los campos del formulario solo funciona cuando
     * la busqueda se la hace enviando parametros al constructor del modulo
     *
     * @param clienteDp
     */
    private void datoscliente(cliente clienteDp) {
    ...
    public void modificarAfectados(boolean facturarProforma, boolean facturarOdt) {

        if (facturarProforma) {

            proformaBdd proformaMD = new proformaBdd();
            int numeroProforma = Integer.parseInt("" + proformaNumero);
            proforma proformaDp = new proforma(numeroProforma, "", "", "", 0,
                    0, 0, User);
            String estado = choice.getSelectedItem() + " Nº: "
                    + jTextFieldNumeroRecibo.getText();
            proformaMD.modificarEstado(conn.getConexion(), proformaDp, ""
                    + estado);//
        }

        if (facturarOdt) {

            ordenDeTrabajo odtDp = new ordenDeTrabajo(0, 0, 0, "", "", 0, "",
                    "", "", 0, 0, 0, "");
            int odtNumero = Integer.parseInt("" + numeroOdt);
            odtDp.setNumeroOrdenTrabajo(odtNumero);
            ordenTrabajoBdd odtMd = new ordenTrabajoBdd();
            Object arr[][] = odtMd.solicitarIdTablaOrdenTraajoPorNumero(conn
                    .getConexion(), "" + odtDp.getNumeroOrdenTrabajo());
            odtMd.modificarEstado(conn.getConexion(), ""
                    + choice.getSelectedItem() + " Nº: "
                    + jTextFieldNumeroRecibo.getText(), "" + arr[0][0],
                    numeroOdt);
    ...
    /**
     * resta del inventario la cantidad de un producto existente
     *
     * @param objeto
     */
    public void restarInventario(reciboDetalle objeto, float cantidad) {


        inventarioBdd inventarioMD = new inventarioBdd();
        float cantidadResta = cantidad;//reservo la cantidad a restar

        Object[][] InventarioDetalle = inventarioMD
                .retornarCantidadLoteItemsInventario(conn.getConexion(), objeto.getIdProducto());
        boolean continuarList = true;
        int i=0;
        while (continuarList) {

                InventarioDetalle inventarioDP1 = new InventarioDetalle(i, i,cantidadResta, cantidadResta, "" + util.fechaActual(),    "" + util.fechaActual());
                inventarioDP1.setIdInventario(Integer.parseInt(""+ InventarioDetalle[i][0]));//
                                                                                        //

                //recoge el total
                //en ese lote
                float cantidadProductoSel = Float.parseFloat(""
                        + InventarioDetalle[i][2]);// cantidad prod en bdd

                                     //cantidad solicitada

///
                if ( continuarList  && cantidadResta!=0 ) {
                    //System.out.println("quedan: "+cantidadResta+ "total: "+cantidadProductoSel+
                        //    "i es:"+i);
                     boolean continuar=true;

                    if(cantidadProductoSel<cantidadResta && continuar){
                            //    System.out.println("es menor");
                                 inventarioDP1.setTotalBruto(0);
                                 cantidadResta=cantidadResta-cantidadProductoSel;
                                 continuar=false;
                   }
                    if((cantidadProductoSel==cantidadResta) && continuar){
                    ...
                    if((cantidadProductoSel>cantidadResta) && continuar){
                      //  System.out.println("es mayor");
                        inventarioDP1.setTotalBruto(cantidadProductoSel-cantidadResta);
                        cantidadResta=cantidadResta-cantidadResta;
                        continuar=false;
                    }

                  //  System.out.println("fin secuencia");
                    inventarioMD.modificarLote(conn.getConexion(),inventarioDP1);



                }
///////////////////////////
                if(cantidadResta==new Float(0.0)){
                    System.out.println("se detubo");
                    continuarList = false;// detengo proceso
                }
    ...
    public boolean cantidadItemsIngresados() {
        float cantidadEnRecibo = 0;
        boolean permitir = true;
        if (modelo.getRowCount() > 0) {
            for (int i = 0; i < modelo.getRowCount(); i++) {
                if (getChoiceProductos().getSelectedItem().toString()
                        .compareTo("" + modelo.getValueAt(i, 1)) == 0)
                    cantidadEnRecibo = Float.parseFloat(""
                            + modelo.getValueAt(i, 0));
            }
        }

        float cantidadIngresada = Float.parseFloat(""
                + util.cortarCadenaString(jTextFieldCantidad.getText().trim(),
                        6));
        try {

            float cantDisponible = devolverMaximoParaIngresar();
            cantidadIngresada = Float.parseFloat(""
                    + jTextFieldCantidad.getText().trim());
            cantidadIngresada = cantidadIngresada + cantidadEnRecibo;
            if (cantidadIngresada > cantDisponible) {
                permitir = false;
            }
        } catch (Exception convertProblem) {
            convertProblem.printStackTrace();
    ...
    /**
     * devuelve la cantidad máxima de un producto
     *
     * @return
     */
    public float devolverMaximoParaIngresar() {
        float cantDisponible = 0;
        try {
            arregloProductosTempHash productoSolicitado = listaProductosDescripcionArreglo
                    .get("" + choiceProductos.getSelectedItem());
            cantDisponible = productoSolicitado.getTotalBruto();
        } catch (Exception convertProblem) {
            convertProblem.printStackTrace();
    ...
    public void enviarPago(Integer tipago, Integer idRecibo, String total,
            String fecha, Integer tipoRecibo) {
        tipoPagoReciboBdd tipoPagoMD = new tipoPagoReciboBdd();
        tipoPagoRecibo tipoPagoReciboDP1 = new tipoPagoRecibo(tipago, idRecibo,
                5, total, fecha, "Pago generado por el sistema", fecha,
                "Pagado", tipoRecibo);

        tipoPagoMD.insertar(conn.getConexion(), tipoPagoReciboDP1);
    }
...
if(choice.getSelectedItem().compareTo("Factura")==0)
    tipo=2;
try{
numero=Integer.parseInt(""+jTextFieldNumeroRecibo.getText().trim());
...
//existe no continuar
String numeroBuscado=reciboMd.ChecarExistenciaRecibo(conn.getConexion(),tipo, numero);

if(numeroBuscado.compareToIgnoreCase(""+numero)==0 )
{
    continuar2=false;
}
...
if(continuar2){
    ...
    if(modelo.getRowCount()!=0)
            limpiar();
    detalleRecibo item=new detalleRecibo(0, "Anulado", 0, 0, true);
    ...
    jButtonAceptar.setEnabled(false);
        ...
        JOptionPane.showMessageDialog(null, "No se puede anular el recibo ingresado !" , "Alerta!",
                JOptionPane.ERROR_MESSAGE);
...
public boolean verificarRecibo(){
    ...
    if (choice.getSelectedItem().compareTo("Factura") == 0)
        tipoReciboTemp=2;
    int numero=Integer.parseInt(""+jTextFieldNumeroRecibo.getText().trim());

    String numeroBuscado=reciboMd.ChecarExistenciaRecibo(conn.getConexion(),tipoReciboTemp, numero);
    //System.out.println("aaa:"+numeroBuscado);
    if(numeroBuscado.compareToIgnoreCase(""+numero)==0 )
    {
        reciboExiste=true;
    }else{
...
public void corregirSubotales(Object productos[][],float iva){
    //TotalODTObtenido
    //contatItems

    float resta1=util.TruncarRedondear(sumatorioSubtotales*iva);
    //resta1=util.Redondear(""+resta1, 2);
if((resta1-TotalODTObtenido)>0.0){
    for(int i=0;i<contatItems;i++)
        poblarTabla(Float.parseFloat(""+productos[i][0]),""+productos[i][1],
                    Float.parseFloat(""+productos[i][2]),Float.parseFloat(""+productos[i][3]));
}else{
    for(int i=0;i<contatItems;i++)
        poblarTabla(Float.parseFloat(""+productos[i][0]),""+productos[i][1],
                    Float.parseFloat(""+productos[i][2]),Float.parseFloat(""+productos[i][3]));
    ...
    public void poblarTabla(float cantidad,String descripcion,float Precio,float subtotal
                          ){
        detalleRecibo item = new detalleRecibo(cantidad,
                descripcion, Precio, subtotal, new Boolean(true));
